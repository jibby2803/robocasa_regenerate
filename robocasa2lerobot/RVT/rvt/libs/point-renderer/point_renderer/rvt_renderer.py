...
class RVTBoxRenderer():
    ...
    def _get_cube_cameras_from_extrinsics(
        self,
        img_size,
        orthographic,
        extrinsics
        ...
        if orthographic:
            cameras = OrthographicCameras.from_rotation_and_translation(R, T, img_sizes_w=[2, 2], img_size_px=img_size)
        else:
            S = torch.ones(extrinsics[0].shape[0], 1, 3, device=self.device)
            cameras = PerspectiveCameras.from_rotation_and_translation(R, T, S, hfov=60, img_size=img_size)
            # TODO
            # cameras.intrinsics[:, 0, 0] = -cameras.intrinsics[:, 0, 0]
            # cameras.intrinsics[:, 1, 1] = -cameras.intrinsics[:, 1, 1]
            # cameras.intrinsics[:, 0, 2] = img_size - 1 - cameras.intrinsics[:, 0, 2]
            # cameras.intrinsics[:, 1, 2] = img_size - 1 - cameras.intrinsics[:, 1, 2]
            
            if self.intrinsics is not None:
                if len(self.intrinsics.shape) == 2:
                    self.intrinsics = self.intrinsics.unsqueeze(0)
                cameras.intrinsics = self.intrinsics.repeat(extrinsics[0].shape[0], 1, 1).contiguous()
        return cameras
    ...
    def __call__(self, pc, feat, fix_cam=False, dyn_cam_info=None, extrinsics=None):

        self._check_device(pc, "pc")
        self._check_device(pc, "feat")

        if extrinsics is not None:
            self.cameras = self._get_cube_cameras_from_extrinsics(self.img_size,
                                                                  self.orthographic,
                                                                  extrinsics)
            self.cameras = self.cameras.to(self.device)
            self.num_img = len(self.cameras)
